...
public function fetch($id)
{
	...
	$data['data'] = $this->Property_area->get_house($id);
    $data['fetch_floor'] = $this->Property_area->fetch_floor($id);
    $data['prop'] = $this->Property_area->get_prop_img($id);
    $data['floor'] = $this->Property_area->get_floor_img($id);
    $data['master'] = $this->Property_area->get_master_img($id);
    $data['other'] = $this->Property_area->get_other_img($id);
    $this->load->view('admin/edit_house',$data);
}
public function fetch_land($id)
{
    $id=$this->uri->segment(3);
    $data['data'] = $this->Property_area->get_land($id);
    $data['files'] = $this->Property_area->get_land_img($id);
    $this->load->view('admin/edit_land',$data);
}
...
$prop_id=$this->uri->segment(3);
    ...
    // print_r($checked);
     ...
     $amenities = implode("#",$checked6);
        ...
        $this->Property_area->up_house(array('id' => $this->uri->segment(3)), $data);
$this->Property_area->delete_floor($prop_id);
...
// $breadth1=1;
       ...
       redirect(base_url().'Edit_property/fetch/'.$prop_id);
}
...
$this->Property_area->up_land(array('id' => $this->uri->segment(3)), $data);
 ...
 if(!empty($_FILES['userFiles5']['name'])){
        ...
        redirect(base_url().'Edit_property/fetch_land/'.$prop_id);

   
}
}

public function remove_land_files(){
    $id=$this->uri->segment(3);
    $id1=$this->uri->segment(4);
    //$remove_file = $this->file->remove_file($id);
    $this->Property_area->remove_land_file($id);
    redirect('Edit_property/fetch_land/'.$id1);
    }
public function remove_prop_files(){
    $id=$this->uri->segment(3);
    $id1=$this->uri->segment(4);
    //$remove_file = $this->file->remove_file($id);
    $this->Property_area->remove_land_file($id);
    redirect('Edit_property/fetch/'.$id1);
    }
    public function remove_floor_files(){
    $id=$this->uri->segment(3);
    $id1=$this->uri->segment(4);
    //$remove_file = $this->file->remove_file($id);
    $this->Property_area->remove_land_file($id);
    ...
    public function remove_master_files(){
    $id=$this->uri->segment(3);
    $id1=$this->uri->segment(4);
    //$remove_file = $this->file->remove_file($id);
    $this->Property_area->remove_land_file($id);
    ...
    public function remove_other_files(){
    $id=$this->uri->segment(3);
    $id1=$this->uri->segment(4);
    //$remove_file = $this->file->remove_file($id);
    $this->Property_area->remove_land_file($id);
...
public function delete_land($id)
    ...
    public function land_property()
    {
        ...
        foreach ($list as $land) {
            ...
            $row[] = $land->prop_named;
            $row[] = $land->regaddress;
            $row[] = '<button class="btn btn-danger" title="Delete" onclick="delete_house('."'".$land->id."'".')"><i class="glyphicon glyphicon-trash"></i></button>';
